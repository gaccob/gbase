...
// return -1, parse fail
int32_t _wsconn_parse_request(_wsconn_frame_t* frame, char* data, int32_t sz)
{
    ...
    while (*q++ == ' ');
    int32_t i = 0;
    while ((frame->location[i++] = *q++) != ' ' && i < sizeof(frame->location));
...
int32_t _wsconn_proc_sha1_req(struct wsconn_t* con, struct _wsconn_frame_t* frame)
{
    ...
    snprintf(key, "%s258EAFA5-E914-47DA-95CA-C5AB0DC85B11", frame->key);
