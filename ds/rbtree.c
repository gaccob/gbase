...
struct rbtree_node_t* _rbtree_next_node(struct rbtree_node_t* node)
{
    ...
    /* or first parent whose left child tree contains nd */
    source = node;
    dest = node->parent;
...
struct rbtree_node_t* _rbtree_prev_node(struct rbtree_node_t* node)
{
    ...
    /* or first parent whose right child tree contains nd */
    source = node;
    dest = node->parent;
...
struct rbtree_node_t* _rbtree_find_node(struct rbtree_t* tree, struct rbtree_node_t* node, void* data)
{
    int ret;
    if(!tree || !node || !data)
        return NULL;
...
void _rbtree_delete_adjust(struct rbtree_t* tree, struct rbtree_node_t* node, struct rbtree_node_t* parent)
{
    ...
    /* x is deleted & x is BLACK, rebel "from every node to its descendent leaf, there're same black nodes in the path" */
